...
# --- 필요 시 여기만 수정 (하위 run_*.sh 에 export 전달) ---
export SEQ_LEN="${SEQ_LEN:-168}" #720
...
export TRAIN_WINDOW_STRIDE="${TRAIN_WINDOW_STRIDE:-24}"
export SKIP_IF_DONE=0
# export NO_MIDNIGHT_WINDOW_ALIGN=1   # 첫 자정 앵커 끄고 행 0·stride 그리드만 쓸 때

echo "[run_train] SEQ_LEN=${SEQ_LEN} NUM_WORKERS=${NUM_WORKERS} LOG_BATCH_EVERY=${LOG_BATCH_EVERY:-<미설정·스크립트 기본>} BATCH_SIZE=${BATCH_SIZE:-<미설정·스크립트 기본>} TRAIN_WINDOW_STRIDE=${TRAIN_WINDOW_STRIDE} NO_MIDNIGHT_WINDOW_ALIGN=${NO_MIDNIGHT_WINDOW_ALIGN:-0}"
echo "[run_train] FEATURE_MART=${FEATURE_MART:-<미설정·스크립트 기본>}"
