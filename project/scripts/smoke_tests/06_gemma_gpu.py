...
setup_log('06_gemma_gpu')

gemma_path = '/models/gemma-4-e2b-it'
print(f'Gemma 4 E2B 모델 로드 시작: {gemma_path}', flush=True)
...
# Gemma 4: vision encoder의 Gemma4ClippableLinear 충돌 방지
# → regex 패턴으로 language_model 하위 레이어만 타겟팅
# PEFT는 target_modules가 단일 str이면 re.fullmatch 로 처리
TARGET_REGEX = r"model\.language_model\.layers\.\d+\.self_attn\.(q_proj|v_proj)"

print(f'LoRA 타겟 regex: {TARGET_REGEX}', flush=True)
